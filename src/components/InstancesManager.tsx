...
const resolveFloatingCardPlatformId = (): PlatformId => "codex";
...
export function InstancesManager<TAccount extends AccountLike>({
  ...
  const isCodexApp = true;
  const supportsLaunchModeSelect = true;
  const resolveInstanceLaunchMode = (
    instance?: InstanceProfile | null,
  ): InstanceLaunchMode => instance?.launchMode ?? "app";
  const usesTerminalLaunch = (instance: InstanceProfile) =>
    resolveInstanceLaunchMode(instance) === "cli";
  const supportsStopControl = instances.some((item) => !usesTerminalLaunch(item));
  const hidePathFieldInEditModal = false;
  const showWorkingDirField = formLaunchMode === "cli";
  const floatingCardPlatformId = useMemo(() => resolveFloatingCardPlatformId(), []);
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <div className="instances-list instances-list-codex">
