...
use crate::modules::{codex_oauth, config, logger};
...
fn get_accounts_storage_path() -> PathBuf {
    let data_dir = config::get_data_dir().unwrap_or_else(|_| {
        dirs::home_dir()
            .expect("无法获取用户目录")
...
fn repair_account_index_from_details(reason: &str) -> Option<CodexAccountIndex> {
    ...
    for entry in entries.flatten() {
        ...
        let Some(account_id) = path.file_stem().and_then(|item| item.to_str()) else {
            continue;
        };
    ...
    let backup_path = if index_path.exists() {
        let timestamp = chrono::Utc::now().format("%Y%m%d%H%M%S");
        let backup = index_path.with_extension(format!("json.repair-{}.bak", timestamp));
        match fs::copy(&index_path, &backup) {
            Ok(_) => Some(backup),
            Err(err) => {
                logger::log_warn(&format!(
...
pub fn set_current_account_id(account_id: &str) -> Result<CodexAccount, String> {
...
#[serde(rename_all = "camelCase")]
pub struct CodexQuotaAlertPayload {
    pub platform: String,
...
pub fn run_quota_alert_if_needed(
) -> Result<Option<CodexQuotaAlertPayload>, String> {
    let cfg = crate::modules::config::get_user_config();
    ...
    let payload = CodexQuotaAlertPayload {
    ...
    dispatch_codex_quota_alert(&payload);
    Ok(Some(payload))
}
