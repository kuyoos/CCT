    ...
    match msg {
        ...
        WsMessage::SwitchAccount {
            ...
            tokio::spawn(async move {
                let switch_result = crate::modules::codex_account::set_current_account_id(&account_id);
                ...
                match switch_result {
                    Ok(account) => {
                        ...
                        broadcast_data_changed("ws_switch_codex_account");
                        if let Some(request_id) = request_id.clone() {
                            let response = WsMessage::SuccessResponse {
                                request_id,
                                message: "切换 Codex 账号成功".to_string(),
                            };
                            if let Ok(json) = serde_json::to_string(&response) {
                                let _ = server_clone.send(json);
                            }
...
fn get_accounts_info() -> Result<(Vec<AccountInfo>, Option<String>), String> {
    ...
    let account_infos: Vec<AccountInfo> = accounts
        ...
        .map(|acc| AccountInfo {
            ...
            subscription_tier: acc.plan_type.clone(),
        })
        .collect();
...
fn get_accounts_with_tokens_info() -> Result<(Vec<AccountTokenInfo>, Option<String>), String> {
    ...
    let account_infos: Vec<AccountTokenInfo> = accounts
        ...
        .map(|acc| AccountTokenInfo {
            ...
            last_used: acc.last_used,
            subscription_tier: acc.plan_type.clone(),
            refresh_token: acc.tokens.refresh_token.clone().unwrap_or_default(),
            access_token: acc.tokens.access_token.clone(),
            expires_at: 0,
            project_id: acc.account_id.clone(),
        })
        .collect();
...
fn get_current_account_info() -> Result<Option<AccountInfo>, String> {
    ...
    Ok(current.map(|acc| AccountInfo {
        id: acc.id.clone(),
        email: acc.email.clone(),
        name: acc.account_name.clone(),
        is_current: current_id.as_ref() == Some(&acc.id),
...
fn handle_add_account(
    ...
    Err(format!(
        "Codex-only 版本不支持通过旧扩展协议同步账号，请在应用内导入 Codex 账号: {}",
        email
    ))
}
...
fn handle_delete_account_by_email(email: &str) -> Result<String, String> {
    ...
    match target {
        Some(acc) => {
            codex_account::remove_account(&acc.id)?;
            crate::modules::logger::log_info("[WS] Codex 账号已删除");
            Ok(format!("账号已删除: {}", email))
        }
        None => {
            crate::modules::logger::log_info("[WS] Codex 账号不存在，无需删除");
